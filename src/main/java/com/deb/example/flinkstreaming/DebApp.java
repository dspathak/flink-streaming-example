...
public class DebApp {
	...
	private static int TIMEWINDOW = 5; //in secs
	
	public static void main(String[] args) throws Exception {
		
		final StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
		env.setStreamTimeCharacteristic(TimeCharacteristic.EventTime);
		env.getConfig().setAutoWatermarkInterval(100); //millisecs
...
//				.assignTimestampsAndWatermarks(new PunctuatedAssigner())
				.assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor<LogEvent>(Time.seconds(15)) {
					public long extractTimestamp(LogEvent element) {
