...
public class DebAppV0 {
	...
	public static void main(String[] args) throws Exception {
		...
		// monitor directory, checking for new files every 1000 milliseconds
		TextInputFormat format = new TextInputFormat(new org.apache.flink.core.fs.Path(FILEPATH));
		format.setFilesFilter(FilePathFilter.createDefaultFilter());

		DataStream<LogEvent> inputStream = env.readFile(format, FILEPATH, FileProcessingMode.PROCESS_CONTINUOUSLY, 1000)
				.filter(new FilterFunction<String>() {
					private static final long serialVersionUID = 1L;
					public boolean filter(String line) { if (line.contains("some header identifier")) return false; else return true; }
				})
		...
		DataStream<LogEvent> inputLogEventStream = inputStream				
